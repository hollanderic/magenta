...
fbl::RefPtr<PinnedBuffer> PinnedBuffer::Create(size_t size, zx::bti bti,
...
//create vmar large enough for rx,tx buffers, and rx,tx dma descriptors
    vmar_mgr = fbl::VmarManager::Create(size, nullptr);
    if (!vmar_mgr) {
    ...
    auto pbuf = fbl::AdoptRef(new (&ac) PinnedBuffer(fbl::move(bti)));
    if (!ac.check()) {
        return nullptr;
    }
    ...
    status = pbuf->bti_.pin(ZX_BTI_PERM_READ | ZX_BTI_PERM_WRITE,
...
zx_status_t PinnedBuffer::LookupPhys(size_t offset, zx_paddr_t* out) {
    if (offset >= GetSize()){
        *out = 0;
