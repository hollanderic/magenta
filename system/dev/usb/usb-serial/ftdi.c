...
static uint32_t ftdi_check_state(ftdi_t* ftdi) {
    uint32_t state = 0;
    list_node_t* nd =list_peek_head(&ftdi->free_write_reqs);
    state |= (nd) ? SERIAL_STATE_WRITABLE : 0;
    nd =list_peek_head(&ftdi->completed_reads);
    state |= (nd) ? SERIAL_STATE_READABLE : 0;
